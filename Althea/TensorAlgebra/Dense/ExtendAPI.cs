...
namespace Althea.TensorAlgebra.Dense
{
	...
	public interface IExtendAbstractApi : IAbstractRuntimeApi<IExtendAbstractApi>
	{
		...
		/// <param name="tensor">The dense tensor as a <see cref="DenseTensorWrapper{T, TS}"/> to be in-place modified</param>
		...
		[AbstractApiMethod]
		public abstract bool PointWiseBinary<T, TS>(DenseTensorWrapper<T, TS> tensor, T value, BinaryOperation op) where T : unmanaged, INumber<T> where TS : class, IStorage<T, TS>;
		...
		/// <typeparam name="T">Any unmanaged number as the data type</typeparam>
		/// <typeparam name="TS">The storage type that implements <see cref="IStorage{T, TSelf}"/></typeparam>
		/// <param name="tensor">The dense tensor as a <see cref="DenseTensorWrapper{T, TS}"/></param>
		/// <param name="result">Output the 2-norm result as a <typeparamref name="T"/></param>
		/// <returns>Whether this implementation supports the given parameters or not. If false, further internal operation is not allowed.</returns>
		/// <exception cref="ArgumentNullException">If <paramref name="tensor"/> is invalid</exception>
		[AbstractApiMethod]
		public abstract bool Norm<T, TS>(DenseTensorWrapper<T, TS> tensor, out T result) where T : unmanaged, INumber<T> where TS : class, IStorage<T, TS>;
		...
		/// <exception cref="ArgumentNullException">If <paramref name="tensor"/> is invalid</exception>
		[AbstractApiMethod]
		public abstract bool PointWiseAggregation<T, TS>(DenseTensorWrapper<T, TS> tensor, UnaryOperation op, BinaryOperation reduce, out T result) where T : unmanaged, INumber<T> where TS : class, IStorage<T, TS>;
		...
		[AbstractApiMethod]
		public abstract bool PointWiseArgAggregation<T, TS>(DenseTensorWrapper<T, TS> tensor, UnaryOperation op, BinaryOperation reduce, out long index) where T : unmanaged, INumber<T> where TS : class, IStorage<T, TS>;
	}
}
