...
public readonly ref struct DenseArrayWrapper<T, TS> where T : unmanaged, IBaseNumber<T> where TS : class, IStorage<T, TS>
{
	...
	{
		return this.m_values == other.m_values && this.SizeEquals(other);
	}
	...
	/// </summary>
	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	public static bool operator ==(DenseArrayWrapper<T, TS> left, DenseArrayWrapper<T, TS> right) => left.Equals(right);
	...
	{
		return $"{nameof(DenseArrayWrapper<T, TS>)} {{ {nameof(ValueStorage)} = {this.m_values}, {nameof(Size)} = {this.Size.SpanJoin('x')}, {nameof(OuterSize)} = {this.OuterSize.SpanJoin('x')} }}";
	}
	#endregion
	...
	public DenseArrayWrapper(TS value, ReadOnlySpan<long> size)
	{
		...
		this.m_outerSize = ref size.Ref();
		this.m_strides = ref Unsafe.NullRef<long>();
	}
