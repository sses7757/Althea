...
namespace Althea.Helpers
{
	...
	public interface IFixedBuffer<T> : IEnumerable<T>, IEnumerable, IReadOnlyCollection<T> where T : unmanaged, IEquatable<T>
	{
		/// <summary>
		...
		TStruct ToStruct<TStruct>(uint copyStart = 0) where TStruct : struct;
		...
		/// <exception cref="InvalidOperationException">If the size of <typeparamref name="TStruct"/> is larger than the size of this fixed buffer minus <paramref name="copyStart"/></exception>
		void FromStruct<TStruct>(TStruct @struct, uint copyStart = 0) where TStruct : struct;
	}
	#endregion
	...
	public unsafe struct FixedBuffer_36<T> : IEquatable<FixedBuffer_36<T>>, IFixedBuffer<T> where T : unmanaged, IEquatable<T>
	{
		...
		/// <returns>The created <typeparamref name="TStruct"/></returns>
		/// <exception cref="InvalidOperationException">If the size of <typeparamref name="TStruct"/> is larger than the size of this fixed buffer minus <paramref name="copyStart"/></exception>
		public TStruct ToStruct<TStruct>(uint copyStart = 0) where TStruct : struct
		{
			uint size = (uint)Marshal.SizeOf<TStruct>();
			if (size + copyStart > 36)
				throw new InvalidOperationException(Resources.Other.InvalidGeneric);
			var s = new TStruct();
			Unsafe.CopyBlock(Unsafe.AsPointer(ref s), this.Pointer, size);
			return s;
		}
		...
		/// <param name="copyStart">The start position to copy of this fixed buffer in bytes</param>
		/// <exception cref="InvalidOperationException">If the size of <typeparamref name="TStruct"/> is larger than the size of this fixed buffer minus <paramref name="copyStart"/></exception>
		public void FromStruct<TStruct>(TStruct @struct, uint copyStart = 0) where TStruct : struct
		{
			uint size = (uint)Marshal.SizeOf<TStruct>();
			if (size + copyStart > 36)
				throw new InvalidOperationException(Resources.Other.InvalidGeneric);
			Unsafe.CopyBlock(this.Pointer + copyStart, Unsafe.AsPointer(ref @struct), size);
		}
		#endregion
