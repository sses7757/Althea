...
{
	/// <summary>
	/// The fixed buffer struct of class type <typeparamref name="T"/> with size = __placeholder__.
	/// </summary>
	...
	public struct FixedClassBuffer___placeholder__<T> : IEqualityOperators<FixedClassBuffer___placeholder__<T>>, IReadOnlyList<T>, IAsSpan<T> where T : class
	{
		...
		/// <summary>
		/// Create a <see cref="FixedClassBuffer___placeholder__{T}"/> with a given <paramref name="array"/>
		/// </summary>
		/// <param name="array">The given array of <typeparamref name="T"/> used to create</param>
		/// <exception cref="ArgumentException">If the size of <paramref name="array"/> is larger than __placeholder__</exception>
		/// <exception cref="ArgumentNullException">If <paramref name="array"/> is null</exception>
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public FixedClassBuffer___placeholder__(params T[] array)
		{
		...
		/// <summary>
		/// Create a <see cref="FixedClassBuffer___placeholder__{T}"/> with a given <paramref name="span"/>
		/// </summary>
		/// <param name="span">The given <see cref="ReadOnlySpan{T}"/> of <typeparamref name="T"/> used to create</param>
		/// <exception cref="ArgumentException">If the size of <paramref name="span"/> is larger than __placeholder__</exception>
		/// <exception cref="ArgumentNullException">If <paramref name="span"/> is empty</exception>
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public FixedClassBuffer___placeholder__(ReadOnlySpan<T> span)
		{
			if (span.IsEmpty)
		...
		{
			if (size == 0)
				return MemoryMarshal.CreateSpan(ref Unsafe.As<FixedClassBuffer___placeholder__<T>, T>(ref this), _count);
			// else
			if (size < 0 || size > _count)
				throw new ArgumentOutOfRangeException(nameof(size), size, Resources.Parameter.InvalidValue);
			return MemoryMarshal.CreateSpan(ref Unsafe.As<FixedClassBuffer___placeholder__<T>, T>(ref this), size);
		}
		...
		{
			if (size == 0)
				return MemoryMarshal.CreateSpan(ref Unsafe.As<FixedClassBuffer___placeholder__<T>, TClass>(ref this), _count);
			// else
			if (size < 0 || size > _count)
				throw new ArgumentOutOfRangeException(nameof(size), size, Resources.Parameter.InvalidValue);
			return MemoryMarshal.CreateSpan(ref Unsafe.As<FixedClassBuffer___placeholder__<T>, TClass>(ref this), size);
		}
		#endregion
		...
		/// </summary>
		/// <param name="other">another <see cref="FixedClassBuffer___placeholder__{T}"/> to compare</param>
		/// <returns>this == <paramref name="other"/></returns>
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public bool Equals(FixedClassBuffer___placeholder__<T> other)
		{
		...
		{
			return obj is FixedClassBuffer___placeholder__<T> buffer && this.Equals(buffer);
		}

		/// <summary>
		/// Override <see cref="ValueType.GetHashCode"/> to get the hash code this <see cref="FixedClassBuffer___placeholder__{T}"/>.
		/// </summary>
		...
		/// </summary>
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public static bool operator ==(FixedClassBuffer___placeholder__<T> left, FixedClassBuffer___placeholder__<T> right)
		{
		...
		/// <summary>
		/// Return the string representation of this <see cref="FixedClassBuffer___placeholder__{T}"/>
		/// </summary>
		/// <returns>the string representation of this <see cref="FixedClassBuffer___placeholder__{T}"/></returns>
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public override string ToString()
		{
			return $"Fixed Class-type Buffer [Size=__placeholder__, Type={typeof(T).GetGenericString()}]";
		}
		#endregion
	}
}