...
namespace Althea.Backend.Mkl.LinearAlgebra.Dense.Tests;
...
public unsafe class BlasApiTests
{
	...
	{
		using var s = CpuHelpers.GenerateFloatData(-10, 10, out var host);

		bool success = api.AbsoluteValueArgMax<Float64, MemF64>(s, stride, out long index);
	...
	public void AbsoluteValueArgMaxComplexTest(long stride)
	{
		...
		bool success = api.AbsoluteValueArgMax<Complex<Float64>, MemC64>(s.As<Complex<Float64>>(), stride, out long index);
		Assert.IsTrue(success);
	...
	public void AbsoluteValueArgMinTest(long stride)
	{
		...
		bool success = api.AbsoluteValueArgMin<Float64, MemF64>(s, stride, out long index);
		Assert.IsTrue(success);
	...
	{
		using var s1 = CpuHelpers.GenerateFloatData(-10, 10, out var host1);
		using var s2 = CpuHelpers.GenerateFloatData(-10, 10, out var host2);
	...
	public void ScaleTest(long stride)
	{
		...
		CpuHelpers.CopyToManaged(s, (Float64*)test);
		for (long i = 0; i < s.Length; i += stride)
		{
	...
	public void AddTest(long stride)
	{
		...
		CpuHelpers.CopyToManaged(s2.MakeReference(0, len), (Float64*)test);
	...
	public void GeneralMatrixMultiplyVectorTest(MatrixOperation op, long ld, long stride)
	{
		using var s1 = CpuHelpers.GenerateFloatData(-10, 10, out var host1);
		using var s2 = CpuHelpers.GenerateFloatData(-10, 10, out var host2);
		using var s3 = CpuHelpers.GenerateFloatData(-10, 10, out var host3);
	...
	public void GeneralMatricesAddTest(MatrixOperation op, long n)
	{
		...
		bool success = api.GeneralMatricesAdd(op, default, n, n, (Float64)0.5, s1, n, default, (MemF64?)null, 0, s2, n);
		Assert.IsTrue(success);
