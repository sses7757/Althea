...
public static partial class NativeMethods
{
	...
	internal static extern MklDftError DftiCreateDescriptor1D(out IntPtr handle, MklDftPrecision precision, MklDftDomain domain, int dim, int length);

	[LibraryImport(Mkl.NativeMethods.MKL_DLL_NAME, EntryPoint = "DftiCreateDescriptor")]
	internal static partial MklDftError DftiCreateDescriptorND(out IntPtr handle, MklDftPrecision precision, MklDftDomain domain, int dim, ReadOnlySpan<int> size);

	[DllImport(Mkl.NativeMethods.MKL_DLL_NAME, EntryPoint = "DftiSetValue")]
	internal static extern MklDftError DftiSetForwardScale(IntPtr handle, MKlDftParam param = MKlDftParam.ForwardScale, double scale = 1.0);

	[DllImport(Mkl.NativeMethods.MKL_DLL_NAME, EntryPoint = "DftiSetValue")]
	internal static extern MklDftError DftiSetBackwardScale(IntPtr handle, MKlDftParam param = MKlDftParam.BackwardScale, double scale = 1.0);

	[DllImport(Mkl.NativeMethods.MKL_DLL_NAME, EntryPoint = "DftiSetValue")]
	internal static extern MklDftError DftiSetForwardScale(IntPtr handle, MKlDftParam param = MKlDftParam.ForwardScale, float scale = 1.0f);

	[DllImport(Mkl.NativeMethods.MKL_DLL_NAME, EntryPoint = "DftiSetValue")]
	internal static extern MklDftError DftiSetBackwardScale(IntPtr handle, MKlDftParam param = MKlDftParam.BackwardScale, float scale = 1.0f);

	[DllImport(Mkl.NativeMethods.MKL_DLL_NAME, EntryPoint = "DftiSetValue")]
	internal static extern MklDftError DftiSetComplexStorage(IntPtr handle, MKlDftParam param = MKlDftParam.ComplexStorage, MklDftStorage storage = MklDftStorage.ComplexAsComplex);

	[DllImport(Mkl.NativeMethods.MKL_DLL_NAME, EntryPoint = "DftiSetValue")]
	internal static extern MklDftError DftiSetConjugateEvenStorage(IntPtr handle, MKlDftParam param = MKlDftParam.ConjugateEvenStorage, MklDftStorage storage = MklDftStorage.ComplexAsComplex);

	[DllImport(Mkl.NativeMethods.MKL_DLL_NAME, EntryPoint = "DftiSetValue")]
	internal static extern MklDftError DftiSetPlacement(IntPtr handle, MKlDftParam param = MKlDftParam.Placement, MklDftPlacement placement = MklDftPlacement.InPlace);

	[LibraryImport(Mkl.NativeMethods.MKL_DLL_NAME, EntryPoint = "DftiSetValue")]
	internal static partial MklDftError DftiSetInputStride(IntPtr handle, MKlDftParam param = MKlDftParam.InputStride, ReadOnlySpan<int> strides = default);

	[LibraryImport(Mkl.NativeMethods.MKL_DLL_NAME, EntryPoint = "DftiSetValue")]
	internal static partial MklDftError DftiSetOutputStride(IntPtr handle, MKlDftParam param = MKlDftParam.OutputStride, ReadOnlySpan<int> strides = default);
...
internal record struct DftiDescriptor : IDisposable
{
	...
	private DftiDescriptor(bool single, bool complex, bool complexAsReal, bool inPlace, bool forward, double scale, int dim, ReadOnlySpan<long> size, ReadOnlySpan<long> inStride, ReadOnlySpan<long> outStride)
	{
		if (dim == 1)
		{
			NativeMethods.DftiCreateDescriptor1D(out this.handle, single ? MklDftPrecision.Single : MklDftPrecision.Double, complex ? MklDftDomain.Complex : MklDftDomain.Real, 1, (int)size[0]).Check();
		}
		else
		{
			Span<int> sizeIn = stackalloc int[dim];
			size.CopyTo(sizeIn, static s => (int)s);
			NativeMethods.DftiCreateDescriptorND(out this.handle, single ? MklDftPrecision.Single : MklDftPrecision.Double, complex ? MklDftDomain.Complex : MklDftDomain.Real, dim, sizeIn).Check();
		}
		try
		{
			Span<int> strideIn = stackalloc int[dim], strideOut = stackalloc int[dim];
			NativeMethods.DftiSetPlacement(this.handle, placement: inPlace ? MklDftPlacement.InPlace : MklDftPlacement.OutOfPlace).Check();
			if (!inStride.IsEmpty)
			{
				inStride.CopyTo(strideIn, static s => (int)s);
				NativeMethods.DftiSetInputStride(this.handle, strides: strideIn).Check();
			}
			if (!outStride.IsEmpty)
			{
				outStride.CopyTo(strideOut, static s => (int)s);
				NativeMethods.DftiSetOutputStride(this.handle, strides: strideOut).Check();
			}
			if (scale != 1)
			{
				if (single)
			...
			NativeMethods.DftiCommitDescriptor(this.handle).Check();
		}
		catch (Exception)
		{
