	...
	internal static class MemoryPointerChecker
	{
		...
		public static unsafe bool GetPointer<T, TInd, TS, TSInd>(ISparseArray<T, TInd, TS, TSInd> matrix, out T* pointer, out MklInt* pointerRow, out MklInt* pointerCol, out long nnz, [CallerArgumentExpression("matrix")] string? matrixName = null) where T : unmanaged, IBaseNumber<T> where TS : class, IStorage<T, TS> where TInd : unmanaged, IBinaryInt<TInd> where TSInd : class, IStorage<TInd, TSInd>
		{
			...
			if (matrix.IndexStorages.Length != 2 || matrix.IndexStorages[0] is not PureStorage<TInd, CpuMemoryPointer> psRow || matrix.IndexStorages[1] is not PureStorage<TInd, CpuMemoryPointer> psCol)
				return false;
			pointerRow = (MklInt*)psRow.Pointer.Pointer.UnmangedPointer<TInd>(psRow.Pointer.OffsetInBytes);
			pointerCol = (MklInt*)psCol.Pointer.Pointer.UnmangedPointer<TInd>(psCol.Pointer.OffsetInBytes);
