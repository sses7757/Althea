...
public unsafe partial class Api
{
	...
	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	private static Complex<Float32> VectorAbsMinMaxCompex<Test>(Complex<Float32>* x, int length)
	{
		bool doMax = typeof(Test) == typeof(int);
		// initialize
		Vector256<float> extremes = ComplexSquareAbsNoOrderSingle(x);
		...
		{
			Vector256<float> squares = ComplexSquareAbsNoOrderSingle(x + offset);
			extremes = doMax ? Avx.Max(squares, extremes) : Avx.Min(squares, extremes);
			lengthLeft -= Vector256<float>.Count;
			offset += Vector256<float>.Count;
		}
		// reduce main
		...
		return (Float32)MathF.Sqrt(extreme);
	}

	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	private static Complex<Float64> VectorAbsMinMaxCompex<Test>(Complex<Float64>* x, int length)
	{
		bool doMax = typeof(Test) == typeof(int);
		// initialize
		Vector256<double> extremes = ComplexSquareAbsNoOrderDouble(x);
		...
		return (Float64)Math.Sqrt(extreme);
	}
	...
	internal static bool MinMax<T, TS, Test>(TS x, long strideX, out T extreme) where T : unmanaged, IBaseNumber<T> where TS : class, IStorage<T, TS>
	{
		...
		if (T.IsComplexType)
		{
			...
			{
				var temp = VectorAbsMinMaxCompex<Test>((Complex<Float32>*)px, length);
				extreme = *(T*)&temp;
			}
			else // double
			{
				var temp = VectorAbsMinMaxCompex<Test>((Complex<Float64>*)px, length);
				extreme = *(T*)&temp;
			}
		}
		else
	...
	private static void VectorAbsSumProdCompexSingle<Test>(Complex<Float32>* x, int length, out Complex<Float32> aggregation)
	{
		...
		else
		{
			...
			if (!doSum)
			{
				var a0 = aggregate1; var a1 = aggregate2;
				ComplexUnpack(a0, a1, out aggregate1, out aggregate2);
			}
			// loop
			int lengthLeft = length - Vector256<float>.Count, offset = Vector256<float>.Count;
			while (lengthLeft >= Vector256<float>.Count) // Vector256<Complex<Float64>>.Count * 2
			{
				...
				{   // aggregate1 = reals, aggregate2 = imaginaries
					var realA = aggregate1; var imagA = aggregate2;
					ComplexUnpack(current1, current2, out var realB, out var imagB);
					ComplexMultiplyUnpacked<byte>(realA, imagA, realB, imagB, ref aggregate1, ref aggregate2);
				}
				lengthLeft -= Vector256<float>.Count;
				offset += Vector256<float>.Count;
			}
	...
	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	private static void VectorParSumProdManaged<T, Sum>(T* x, int incx, T* y, int incy, int length, bool inclusive) where T : unmanaged, IBaseNumber<T>
	{
		bool doSum = typeof(Sum) == typeof(bool);

		if (!inclusive)
			length--;
		T result = inclusive ? x[0] : doSum ? T.Zero : T.One;
		for (int i = inclusive ? 1 : 0, ix = inclusive ? incx : 0, iy = incy; i < length; i++, ix += incx, iy += incy)
		{
			T v = x[ix];
			result = doSum ? result + v : result * v;
			y[iy] = result;
		}
	}
	...
	internal static bool ParSumProd<T, TS1, TS2, Sum>(TS1 x, long strideX, TS2 y, long strideY, bool inclusive) where T : unmanaged, IBaseNumber<T> where TS1 : class, IStorage<T, TS1> where TS2 : class, IStorage<T, TS2>
	{
		...
		if (length == 0)
			return true;
		py[0] = inclusive ? px[0] : typeof(Sum) == typeof(bool) ? T.Zero : T.One;
		if (length == 1)
			return true;
		// normal case
		VectorParSumProdManaged<T, Sum>(px, incx, py, incy, length, inclusive);
		return true;
	}
	#endregion
