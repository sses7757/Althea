...
namespace Althea.Backend.Arrays
{
	...
	public class SparseTensor<T, TInd> : BaseSparseTensor<T, TInd>, IKrylovVector<SparseTensor<T, TInd>, T> where T : unmanaged where TInd : unmanaged
	{
		...
		public override SparseTensor<T, TInd> AddTensor(T scalarThis, BaseTensor<T> other, T scalarOther)
		{
		...
		/// <exception cref="ArgumentOutOfRangeException">If <paramref name="scalar"/> is 0</exception>
		public override SparseTensor<T, TInd> Contract(BaseTensor<T> other, T scalar, ReadOnlySpan<char> outputLabels = default)
		{
			if (scalar.IsZero())
				throw new ArgumentOutOfRangeException(nameof(scalar), scalar, Resources.Parameter.CannotZero);
			if (other is not SparseTensor<T, TInd> sparse)
		...
		/// <summary>
		/// Get other requisite informations for re-constructing the array of that derived class type. Only returns the <see cref="BaseTensor{T}.Labels"/> and <see cref="Althea.Arrays.BaseSparseTensor{T, TInd}.DefaultValue"/>.
		/// </summary>
