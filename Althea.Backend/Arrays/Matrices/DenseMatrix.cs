...
namespace Althea.Backend.Arrays
{
	...
	internal interface IMatrix<T> where T : unmanaged
	{
		...
		IBaseMatrix<T> AddMatrix(T scalarThis, T scalarOther, IBaseMatrix<T> other, MatrixOperation opThis = MatrixOperation.None, MatrixOperation opOther = MatrixOperation.None);

		IBaseMatrix<T> ApplyOperation(MatrixOperation operation);

		IBaseMatrix<T> MultiplyMatrix(T scalar, IBaseMatrix<T> other, MatrixOperation opThis = MatrixOperation.None, MatrixOperation opOther = MatrixOperation.None);
		...
		(long m, long n) CheckAdd(T scalarThis, T scalarOther, IBaseMatrix<T> other, ref MatrixOperation opThis, ref MatrixOperation opOther)
		{
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		(long m, long n, long k) CheckMultiply(T scalar, IBaseMatrix<T> other, ref MatrixOperation opThis, ref MatrixOperation opOther)
		{
			if (other is null || !other.IsValid())
				throw new ArgumentNullException(nameof(other));
			if (scalar.IsZero())
	...
	internal interface IDenseMatrix<T> : IMatrix<T> where T : unmanaged
	{
		...
		void OverwriteByMatricesProduct(T α, IBaseMatrix<T> A, IBaseMatrix<T> B, T β = default, MatrixOperation opA = MatrixOperation.None, MatrixOperation opB = MatrixOperation.None);

		void OverwriteByMatricesSum(IBaseMatrix<T>? A, IBaseMatrix<T>? B, T scalarA = default, T scalarB = default, MatrixOperation opA = MatrixOperation.None, MatrixOperation opB = MatrixOperation.None);
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		(long m, long n, bool nullA, bool nullB) CheckOverwriteBySum(ref IBaseMatrix<T>? A, ref IBaseMatrix<T>? B, T scalarA, T scalarB, ref MatrixOperation opA, ref MatrixOperation opB)
		{
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		(long m, long n, long k) CheckOverwriteByProduct(T α, IBaseMatrix<T> A, IBaseMatrix<T> B, ref MatrixOperation opA, ref MatrixOperation opB)
		{
			if (α.IsZero())
		...
		where T : unmanaged
	{
		...
		/// <param name="offsetCol">The starting offset of the columns to take</param>
		/// <param name="countCol">The number of the columns to take</param>
		/// <param name="overwrite">The <see cref="IBaseMatrix{T}"/> to be overwritten</param>
		/// <exception cref="ArgumentOutOfRangeException">If <paramref name="offsetRow"/> or <paramref name="countRow"/> or <paramref name="offsetCol"/> or <paramref name="countCol"/> is out of range</exception>
		/// <exception cref="ArgumentException">If <paramref name="overwrite"/> cannot be overwritten</exception>
		public override void GetSubmatrix(long offsetRow, long countRow, long offsetCol, long countCol, IBaseMatrix<T> overwrite)
		{
		...
		/// <param name="offsetCol">The starting offset of the columns to take</param>
		/// <param name="countCol">The number of the columns to take</param>
		/// <param name="value">The <see cref="IBaseMatrix{T}"/> whose value will overwrite this matrix from (<paramref name="offsetRow"/>, <paramref name="countRow"/>) with size (<paramref name="countRow"/>, <paramref name="countCol"/>)</param>
		...
		public override void SetSubmatrix(long offsetRow, long countRow, long offsetCol, long countCol, IBaseMatrix<T> value)
		{
		...
		/// <remarks>If <see cref="IBaseMatrix{T}.NRows"/> != <see cref="LeadDim"/>, a new vector will be created to return.</remarks>
		...
		/// <remarks>If the computed new number of rows is not <see cref="IBaseMatrix{T}.NRows"/> and <see cref="IBaseMatrix{T}.NRows"/> != <see cref="LeadDim"/>, a new matrix will be created to return.</remarks>
		...
		/// <exception cref="InvalidOperationException">If the <see cref="LeadDim"/> != <see cref="IBaseMatrix{T}.NRows"/></exception>
		...
		/// <summary>
		/// Create a new <see cref="IBaseMatrix{T}"/> which is the simple operation result of this matrix under <paramref name="operation"/>.
		/// </summary>
		...
		/// <param name="other">The input right <see cref="IBaseMatrix{T}"/> to be added</param>
		...
		public override DenseMatrix<T> AddMatrix(T scalarThis, T scalarOther, IBaseMatrix<T> other, MatrixOperation opThis = MatrixOperation.None, MatrixOperation opOther = MatrixOperation.None)
		{
		...
		/// <summary>
		/// Create a new <see cref="IBaseMatrix{T}"/> which is the multiplication result of this matrix and the <paramref name="other"/> matrix.
		/// </summary>
		/// <param name="scalar">The scalar to multiply to the result</param>
		/// <param name="other">The input right <see cref="IBaseMatrix{T}"/> to be multiplied</param>
		/// <param name="opThis">The <see cref="MatrixOperation"/> to apply to this matrix before addition</param>
		/// <param name="opOther">The <see cref="MatrixOperation"/> to apply to the <paramref name="other"/> matrix before addition</param>
		/// <returns>A new <see cref="IBaseMatrix{T}"/> as the result of <c><paramref name="scalar"/> * <paramref name="opThis"/>(this) * <paramref name="opOther"/>(<paramref name="other"/>)</c></returns>
		/// <exception cref="ArgumentNullException">If <paramref name="other"/> is null or empty</exception>
		...
		public override IBaseMatrix<T> MultiplyMatrix(T scalar, IBaseMatrix<T> other, MatrixOperation opThis = MatrixOperation.None, MatrixOperation opOther = MatrixOperation.None)
		{
		...
		public void OverwriteByMatricesSum(IBaseMatrix<T>? A, IBaseMatrix<T>? B, T scalarA = default, T scalarB = default, MatrixOperation opA = MatrixOperation.None, MatrixOperation opB = MatrixOperation.None)
		{
		...
		/// <exception cref="NotSupportedException">If <paramref name="A"/> or <paramref name="B"/> is neither a <see cref="DenseMatrix{T}"/> nor a <see cref="SymmetricDenseMatrix{T}"/> nor a <see cref="ISparseMatrix{T}"/></exception>
		public void OverwriteByMatricesProduct(T α, IBaseMatrix<T> A, IBaseMatrix<T> B, T β = default, MatrixOperation opA = MatrixOperation.None, MatrixOperation opB = MatrixOperation.None)
		{
		...
		#region print
		internal static string ActualPrint(Storage<T> storage, long actualRow, long actualCol, long ld, PrintSettings settings, string? prefix = null, string? postfix = null)
		{
			// get managed array
			...
			string str = managed.ToMatrixString(rows, more: actualCol - cols, precision: settings.Precision, prefix, postfix);
			if (actualRow > rows)
