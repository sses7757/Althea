...
internal readonly unsafe ref struct CudaBuffer
{
	...
	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	public static unsafe MatBuffer<T> Create<T>(T* org, long ld, long n) where T : unmanaged => new(org, ld, n);

	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	public static unsafe VecBuffer<T> Create<T>(T* org, long n) where T : unmanaged => new(org, n);

	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	public static unsafe VecBuffer<T> Create<T>(T* org) where T : unmanaged => new(org, 0);

	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	public static unsafe VecBuffer<T> Create<T>(long n) where T : unmanaged => new(null, n);
}


internal readonly unsafe ref struct MatBuffer<T> where T : unmanaged
{
	private readonly long ld;
	...
	internal MatBuffer(T* org, long ld, long n)
	{
		if (org == null)
		{
			...
			NativeMethods.cudaMalloc(out var p, bytes).Check();
			this.ptr = (T*)p;
			this.ld = n;
		}
		else
	...
	public void Dispose()
	{
		if (this.ld > 0)
		{
			NativeMethods.cudaFree(this.ptr).Check();
		}
	}

	public readonly long LD
	{
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		get => Math.Abs(this.ld);
	}
...
internal readonly unsafe ref struct VecBuffer<T> where T : unmanaged
{
	...
	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	internal VecBuffer(T* org, long n)
	{
		if (org == null)
		{
			long bytes = n * sizeof(T);
			NativeMethods.cudaMalloc(out var p, bytes).Check();
			this.ptr = ((IntPtr)p).ToInt64();
		}
		else
		{
			this.ptr = -((IntPtr)org).ToInt64();
		}
	}
	...
	public void Dispose()
	{
		if (this.ptr > 0)
		{
			NativeMethods.cudaFree((void*)(IntPtr)this.ptr).Check();
		}
	}

	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	public static implicit operator T*(VecBuffer<T> buffer) => (T*)(IntPtr)(-buffer.ptr);
}