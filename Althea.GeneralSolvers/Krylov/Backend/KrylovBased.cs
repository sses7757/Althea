...
namespace Althea.GeneralSolvers.Krylov.Backend
{
    internal static class KrylovBased
	{
		...
		private static unsafe void GetSchur<T>(int n, SpanMatrix<T> H, Span<T> outVals, Span<T> outValsImag, SpanMatrix<T> outSchurT, SpanMatrix<T> outSchurU, Span<int> outConjugatePairs) where T : unmanaged, IBinaryFloat<T>
		{
			H.CopyTo(outSchurT);
		...
		private static unsafe void ReorderSchur<T, TVec>(Span<byte> select, int preserveCount, SpanMatrix<T> schurT, SpanMatrix<T> schurU, TVec r, ref SpanList<TVec> qs, Span<T> a, T beta) where T : unmanaged, IBinaryFloat<T> where TVec : class, IKrylovVector<T, TVec>
		{
		...
		private static unsafe void SortPairs<T>(int n, WhichEigenvalues which, Span<T> orderedVals, Span<T> orderedValsImag, SpanMatrix<T> orderedVecs, Span<int> conjugatePairs) where T : unmanaged, IBinaryFloat<T>
		{
		...
		private static unsafe int GetConverge<T>(SpanMatrix<T> H, T beta, T tol, WhichEigenvalues which, bool useGap, Span<T> orderedVals, Span<T> orderedValsImag, SpanMatrix<T> orderedVecs, SpanMatrix<T> schurT, SpanMatrix<T> schurU, Span<T> errorBounds) where T : unmanaged, IBinaryFloat<T>
		{
		...
		private static int PreserveSelect<T>(int nEig, int convergedWithin, ReadOnlySpan<T> orderedVals, ReadOnlySpan<T> orderedValsImag, SpanMatrix<T> orderedVecs, IPreserveSelector selector, Span<bool> eigenSelect) where T : unmanaged, IBinaryFloat<T>
		{
