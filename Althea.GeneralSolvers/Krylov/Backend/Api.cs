...
namespace Althea.GeneralSolvers.Krylov.Backend
{
    ...
    public class Api : IAbstractApi
	{
		...
		#region eigen
		/// <inheritdoc/>
		public virtual bool NaiveKrylovSubspaceEigenHermitain<T, TVec>(ref KrylovSubspaceSolveInfo<T, TVec> info, out (double Value, TVec Vector) eigen) where T : unmanaged, IBinaryFloat<T> where TVec : class, IKrylovVector<T, TVec>
		{
			eigen = (0, TVec.Empty);
	...
	internal static class Common
	{
		...
		#region gap
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		internal static T GetGap<T>(T beta, T tol, ReadOnlySpan<T> vals, ReadOnlySpan<T> vecsLastRow, int target = 0, ReadOnlySpan<int> conjugatePairs = default, T normA = default) where T : unmanaged, IBinaryFloat<T>
		{
			if (normA == T.Zero)
