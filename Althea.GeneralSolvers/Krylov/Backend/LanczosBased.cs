...
namespace Althea.GeneralSolvers.Krylov.Backend
{
    internal static class LanczosBased
	{
		...
		internal static (double val, TVec vec) NaiveLanczos<T, TVec>(Func<TVec, TVec> matrixFunction!!, TVec initial!!, int maxIter, bool checkFirst, TimeSpan interval) where T : unmanaged, IBinaryFloat<T> where TVec : class, IKrylovVector<T, TVec>
		{
