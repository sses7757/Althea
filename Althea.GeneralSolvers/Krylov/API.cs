...
namespace Althea.GeneralSolvers.Krylov
{
	...
	public interface IAbstractApi : IAbstractRuntimeApi<IAbstractApi>
	{
		...
		/// <typeparam name="TVec">The concrete vector class type hat implements <see cref="IBaseVector{T, TVec}"/></typeparam>
		...
		[AbstractApiMethod]
		public abstract bool NaiveKrylovSubspaceEigenHermitain<T, TVec>(ref KrylovSubspaceSolveInfo<T, TVec> info, out (T Value, TVec Vector) eigen) where T : unmanaged, IBinaryFloat<T> where TVec : class, IBaseVector<T, TVec>;
		...
		/// <exception cref="ArgumentException">If <paramref name="info"/> contains invalid value</exception>
		[AbstractApiMethod]
		public abstract bool RestartKrylovSubspaceEigen<T, TVec>(bool hermitian, ref KrylovSubspaceSolveInfo<T, TVec> info, out int converged) where T : unmanaged, IBinaryFloat<T> where TVec : class, IBaseVector<T, TVec>;
		...
		/// <exception cref="ArgumentException">If <paramref name="info"/> contains invalid value</exception>
		[AbstractApiMethod]
		public abstract bool RestartKrylovSubspaceLinearSolve<T, TVec>(bool? hermitianOrDefinite, ref KrylovSubspaceSolveInfo<T, TVec> info, out (TVec Vector, double RelativeError) solve) where T : unmanaged, IBinaryFloat<T> where TVec : class, IBaseVector<T, TVec>;
	}
}