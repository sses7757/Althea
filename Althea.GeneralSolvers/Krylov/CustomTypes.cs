...
namespace Althea.GeneralSolvers.Krylov
{
	...
	public interface IPreserveSelector
	{
		...
		Span<int> PreserveSelect<T>(ReadOnlySpan<T> estimateEigvals, ReadOnlySpan<T> estimateEigvalsImag, ReadOnlySpan<T> estimateEigvecs, int nConverged, int nTarget, int maxIter, Span<int> output, bool withConverged = true) where T : unmanaged, IBinaryFloat<T>;
	}
	...
	internal sealed class BuiltInPreserveSelector : IPreserveSelector
	{
		...
		public Span<int> PreserveSelect<T>(ReadOnlySpan<T> estimateEigvals, ReadOnlySpan<T> estimateEigvalsImag, ReadOnlySpan<T> estimateEigvecs, int nConverged, int nTarget, int maxIter, Span<int> output, bool withConverged = true) where T : unmanaged, IBinaryFloat<T>
		{
