...
// to string
inline static std::string datatypeToString(const DataType type)
{
...
#define UNSUPPORT(funcName, dataType, returnType) do { \
		printf("[%s] does not support [%s]!", #funcName, datatypeToString(dataType).c_str()); \
		return returnType(); \
	} while(0)
...
#define AUTO_FLOAT_FUNC(funcName, dataType, returnType, ...) do { \
		...
		case DataType::ComplexSingle: \
			return funcName<complex::complex<float>>(__VA_ARGS__); \
		case DataType::ComplexDouble: \
			return funcName<complex::complex<double>>(__VA_ARGS__); \
		default: \
...
#define AUTO_FLOAT_FUNC(funcName, dataType, returnType, ...) do { \
		...
		case DataType::ComplexLongDouble: \
			return funcName<complex::complex<long double>>(__VA_ARGS__); \
		default: \
			UNSUPPORT(funcName, dataType, returnType); \
		} \
	} while (0)
...
#define AUTO_ALLTYPE_FUNC(funcName, dataType, returnType, ...) do { \
		...
		case DataType::ComplexInt8: \
			return funcName<complex::complex<int8_t>>(__VA_ARGS__); \
		case DataType::ComplexInt16: \
			return funcName<complex::complex<int16_t>>(__VA_ARGS__); \
		case DataType::ComplexInt32: \
			return funcName<complex::complex<int32_t>>(__VA_ARGS__); \
		case DataType::ComplexInt64: \
			return funcName<complex::complex<int64_t>>(__VA_ARGS__); \
		case DataType::ComplexUInt8: \
			return funcName<complex::complex<uint8_t>>(__VA_ARGS__); \
		case DataType::ComplexUInt16: \
			return funcName<complex::complex<uint16_t>>(__VA_ARGS__); \
		case DataType::ComplexUInt32: \
			return funcName<complex::complex<uint32_t>>(__VA_ARGS__); \
		case DataType::ComplexUInt64: \
			return funcName<complex::complex<uint64_t>>(__VA_ARGS__); \
		default: \
			UNSUPPORT(funcName, dataType, returnType); \
		} \
	} while (0)
...
// automatically generate float and integer complex type's corresponding real type switch functions
#ifndef HAS_LDBL
#define AUTO_COMPLEX_REAL_TYPE_FUNC(funcName, dataType, returnType, ...) do { \
		switch (dataType) \
		{ \
		case DataType::ComplexSingle: \
			return funcName<float>(__VA_ARGS__); \
