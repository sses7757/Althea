...
namespace hptt {
	...
	struct micro_kernel
	{
		static void execute(const floatType* RESTRICT A, const size_t lda, floatType* RESTRICT B, const size_t ldb, const floatType alpha, const floatType beta)
		{
...
#include <immintrin.h>
	...
	{
		static void execute(const double* RESTRICT A, const size_t lda, double* RESTRICT B, const size_t ldb, const double alpha, const double beta)
		{
	...
	{
		static void execute(const float* RESTRICT A, const size_t lda, float* RESTRICT B, const size_t ldb, const float alpha, const float beta)
		{
...
#include <arm_neon.h>
	...
	static INLINE void macro_kernel_scalar(const floatType* RESTRICT A, const size_t lda, int blockingA,
		floatType* RESTRICT B, const size_t ldb, int blockingB,
	...
	static INLINE void macro_kernel(const floatType* RESTRICT A, const floatType* RESTRICT Anext, const size_t lda,
		floatType* RESTRICT B, const floatType* RESTRICT Bnext, const size_t ldb,
		const floatType alpha, const floatType beta)
	{
		constexpr int blocking_micro_ = REGISTER_BITS / 8 / sizeof(floatType);
	...
	template<int betaIsZero, typename floatType, bool conjA>
	void transpose_int_scalar(const floatType* RESTRICT A, int sizeStride1A,
		floatType* RESTRICT B, int sizeStride1B, const floatType alpha, const floatType beta, const ComputeNode* plan)
	{
	...
	void transpose_int(const floatType* RESTRICT A, const floatType* RESTRICT Anext,
		floatType* RESTRICT B, const floatType* RESTRICT Bnext, const floatType alpha, const floatType beta,
	...
	void transpose_int_constStride1(const floatType* RESTRICT A, floatType* RESTRICT B, const floatType alpha, const floatType beta,
		const ComputeNode* plan)
	{
		const int32_t end = plan->end - (plan->inc - 1);
...
#pragma vector nontemporal
		...
		conjA_(false)
	{
...
#ifdef _DEBUG
		printVector(perm, dim, "perm");
		printVector(sizeA, dim, "sizeA");
		printVector(outerSizeA, dim, "outerSizeA");
		printVector(outerSizeB, dim, "outerSizeB");
#endif
		...
		int* tmpPerm = (int*)alloca(dim * sizeof(int));
		int* tmpSizeA = (int*)alloca(dim * sizeof(int));
		int* tmpOuterSizeA = (int*)alloca(dim * sizeof(int));
		int* tmpOuterSizeB = (int*)alloca(dim * sizeof(int));
		...
		// this for not const class method
		// initializes dim_, outerSizeA, outerSizeB, sizeA and perm 
		this->skipIndices(tmpSizeA, tmpPerm, tmpOuterSizeA, tmpOuterSizeB, dim);
		this->fuseIndices();

		// delete temps
		////delete[] tmpPerm, tmpSizeA, tmpOuterSizeA, tmpOuterSizeB;

		// initializes lda_ and ldb_
		this->computeLeadingDimensions();
...
////	}
	...
	static void axpy_1D(const floatType* RESTRICT A, floatType* RESTRICT B, const int myStart, const int myEnd, const floatType alpha, const floatType beta, int numThreads)
	{
		if (!betaIsZero)
		{
...
#pragma vector nontemporal
	...
	template<int betaIsZero, typename floatType, bool useStreamingStores, bool spawnThreads, bool conjA>
	static void axpy_2D(const floatType* RESTRICT A, const int lda,
		floatType* RESTRICT B, const int ldb,
	...
	void Transpose<floatType>::execute_expert() const noexcept
	{
	...
	template<typename floatType>
	void Transpose<floatType>::execute() const noexcept
	{
		if (masterPlan_ == nullptr) {
			fprintf(stderr, "[HPTT] ERROR: master plan has not yet been created.\n");
			exit(-1);
	...
	void Transpose<floatType>::getBestParallelismStrategy(std::vector<int>& bestParallelismStrategy) const
	{
		...
		{ // no perfect match has been found
		   ...
		   // Option 1: keep parallelizing non-stride-1 loops only, but allowing load-imbalance
			...
			parallelize(strat1, avail1, totalTasks1, primes1, 0.92F, loopsAllowed);
			...
			if (perm_[0] != 0)
			{
				...
				parallelize(strat2, avail2, totalTasks2, primes2, 0.92F, loopsAllowed);
				if (getLoadBalance(strat2) > 0.92)
				{
					std::copy(strat2.begin(), strat2.end(), bestParallelismStrategy.begin());
					return;
				...
				parallelize(strat1, avail1, totalTasks1, primes1, 0.9F, loopsAllowedStride1);
				parallelize(strat2, avail2, totalTasks2, primes2, 0.8F, loopsAllowed);
	...
	void Transpose<floatType>::skipIndices(const int* sizeA, const int* perm, const int* outerSizeA, const int* outerSizeB, const int dim)
	{
		...
		if (dim_ == 0) {
			...
			// remove gaps in the perm, if required (e.g., perm=3,1,0 -> 2,1,0)
			int currentValue = 0;
			for (int i = 0; i < dim_; ++i) {
	...
	*/
	...
	void Transpose<floatType>::getBestLoopOrder(std::vector<int>& loopOrder) const
	{
		...
		double* costs = new double[dim_ * dim_];
		for (int i = 0; i < dim_; ++i) {
...
////	 */
////	template<typename floatType>
////	float Transpose<floatType>::estimateExecutionTime(const std::shared_ptr<Plan> plan, const floatType* A, floatType* B)
////	{
...
////	}
	...
	template void Transpose<float>::execute_expert<true, true, true>() const;
	template void Transpose<float>::execute_expert<true, false, true>() const;
	template void Transpose<float>::execute_expert<false, true, true>() const;
	template void Transpose<float>::execute_expert<false, false, true>() const;
	template void Transpose<float>::execute_expert<true, true, false>() const;
	template void Transpose<float>::execute_expert<true, false, false>() const;
	template void Transpose<float>::execute_expert<false, true, false>() const;
	template void Transpose<float>::execute_expert<false, false, false>() const;

	template void Transpose<double>::execute_expert<true, true, true>() const;
	template void Transpose<double>::execute_expert<false, true, true>() const;
	template void Transpose<double>::execute_expert<true, false, true>() const;
	template void Transpose<double>::execute_expert<false, false, true>() const;
	template void Transpose<double>::execute_expert<true, true, false>() const;
	template void Transpose<double>::execute_expert<false, true, false>() const;
	template void Transpose<double>::execute_expert<true, false, false>() const;
	template void Transpose<double>::execute_expert<false, false, false>() const;

	template void Transpose<FloatComplex>::execute_expert<true, true, true>() const;
	template void Transpose<FloatComplex>::execute_expert<false, true, true>() const;
	template void Transpose<FloatComplex>::execute_expert<true, false, true>() const;
	template void Transpose<FloatComplex>::execute_expert<false, false, true>() const;
	template void Transpose<FloatComplex>::execute_expert<true, true, false>() const;
	template void Transpose<FloatComplex>::execute_expert<false, true, false>() const;
	template void Transpose<FloatComplex>::execute_expert<true, false, false>() const;
	template void Transpose<FloatComplex>::execute_expert<false, false, false>() const;

	template void Transpose<DoubleComplex>::execute_expert<true, true, true>() const;
	template void Transpose<DoubleComplex>::execute_expert<false, true, true>() const;
	template void Transpose<DoubleComplex>::execute_expert<true, false, true>() const;
	template void Transpose<DoubleComplex>::execute_expert<false, false, true>() const;
	template void Transpose<DoubleComplex>::execute_expert<true, true, false>() const;
	template void Transpose<DoubleComplex>::execute_expert<false, true, false>() const;
	template void Transpose<DoubleComplex>::execute_expert<true, false, false>() const;
	template void Transpose<DoubleComplex>::execute_expert<false, false, false>() const;
