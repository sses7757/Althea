...
namespace hptt {
	...
	public:
		 ...
		 *        should only be used in conjunction with addThreadId().
		 */
		...
		*/
		...
		void execute_expert() const noexcept;
	...
	private:
		 ...
		 * Private members
		 ***************************************************/
		const floatType* RESTRICT A_; //!< raw data pointer for A
		floatType* RESTRICT B_; //!< raw data pointer for B
		...
		std::vector<int> threadIds_; //!< OpenMP threadIds of the threads involved in the transposition
		int numThreads_;
